 ...
 * OVERCLAIMED: this function's OWN code introduces no non-determinism —
 * no `Date.now()`, no `Math.random()`, no reliance on `Set`/`Map`
 * iteration order to build output (`effect-validation.ts` and
 ...
 * So the true, checkable claim is: DETECTS, DOES NOT STRUCTURALLY
 * PREVENT, adapter-level impurity — `__tests__/purity.test.ts` proves the
 * positive case (a real deterministic adapter run twice deep-equals
 * itself, with THIS function contributing zero non-determinism of its
 * own) and DEMONSTRATES the gap honestly with a deliberately-impure
 * adapter (one that calls `Math.random()`) whose two `simulate()` calls
 * do NOT deep-equal each other — proof that this engine neither adds nor
 * hides that impurity, rather than a claim that it was prevented.
 *
