...
compensation expressed as the same typed array of deltas, applied by one small, total interpreter — turns
both claims from sentences to trust into data to re-run. Today's "undo" is an uninspectable closure, or a
...
once and never checked again, while a rollback record is checked every time it is used.
...
replay, carries the actions that matter most. And the guard keeping this honest is never finished: the check
policing "never reaches an LLM or the network" took seven bypasses here alone, the latest a real parser
still silently defeated by a syntax error. Provable undo is a property of a system under sustained pressure,
not a design doc's one-time claim.
